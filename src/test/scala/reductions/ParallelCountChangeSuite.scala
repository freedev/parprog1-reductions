...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("test moneyThreshold should work") {
    def check(startingMoney: Int, money: Int, coins: List[Int], expected: Boolean) =
      assert(moneyThreshold(startingMoney).apply(money, coins) == expected,
        s"moneyThreshold($startingMoney).apply($money, $coins) should be $expected")
